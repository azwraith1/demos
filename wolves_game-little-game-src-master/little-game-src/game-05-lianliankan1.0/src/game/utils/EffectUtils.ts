...
module EffectUtils {
    ...
    export function showAlphaEffect(obj, time) {
    ...
    //obj         闪烁对象
    //interval    闪烁总工时间
    export function blinkEffect(obj, interval: number = 1000, loop: boolean = false): BitmapBlink {
        return new BitmapBlink(obj, interval, loop);
    }
    ...
    */
    export function playEffect(obj, cartoonType: number = 1): void {
        ...
        var onComplete1: Function = function () {
            if (cartoonType == 1) {
                egret.Tween.get(obj).to({ scaleX: 1, scaleY: 1 }, 500, egret.Ease.elasticOut).call(onComplete2, this);
